...
bool CTransaction::ExecuteTx(int nIndex, CAccountViewCache &cAccountViewCache, CValidationState &cValidationState,
	...
	if (EM_CONTRACT_TX == m_chTxType) {
		...
		tuple<bool, uint64_t, string> ret = vmRunEvn.run(pTx, cAccountViewCache, cScriptDB, nHeight, ullEl,
				m_ullRunStep);
		if (!std::get<0>(ret)) {
			return cValidationState.DoS(100,
...
Object CTransaction::ToJSON(const CAccountViewCache &cAccountView) const{
	Object result;
	CAccountViewCache cAcctView(cAccountView);
    CKeyID cKeyId;
...
bool CTransaction::CheckTransction(CValidationState &cValidationState, CAccountViewCache &cAccountViewCache,
		CScriptDBViewCache &cScriptDB) {
	if (m_cSrcRegId.type() != typeid(CRegID)) {
		return cValidationState.DoS(100, ERRORMSG("CheckTransaction() : CTransaction srcRegId must be CRegID"),
				REJECT_INVALID, "srcaddr-type-error");
	}

	if ((m_cDesUserId.type() != typeid(CRegID)) && (m_cDesUserId.type() != typeid(CKeyID))) {
		return cValidationState.DoS(100,
				ERRORMSG("CheckTransaction() : CTransaction desUserId must be CRegID or CKeyID"), REJECT_INVALID,
				"desaddr-type-error");
	}

	if (!MoneyRange(m_ullFees)) {
		return cValidationState.DoS(100,
				ERRORMSG("CheckTransaction() : CTransaction CheckTransction, appeal tx fee out of range"),
	...
	if (!cAccountViewCache.GetAccount(boost::get<CRegID>(m_cSrcRegId), cAcctInfo)) {
		return cValidationState.DoS(100,
				ERRORMSG("CheckTransaction() :CTransaction CheckTransction, read account falied, regid=%s",
						boost::get<CRegID>(m_cSrcRegId).ToString()), REJECT_INVALID, "bad-getaccount");
	}
	if (!cAcctInfo.IsRegister()) {
		return cValidationState.DoS(100,
				ERRORMSG("CheckTransaction(): CTransaction CheckTransction, account have not registed public key"),
	...
	if (!CheckSignScript(cSighash, m_vchSignature, cAcctInfo.m_cPublicKey)) {
		return cValidationState.DoS(100,
				ERRORMSG("CheckTransaction() : CTransaction CheckTransction, CheckSignScript failed"), REJECT_INVALID,
...
uint256 CTransaction::SignatureHash() const {
	...
	cSs << VARINT(m_nVersion) << m_chTxType << VARINT(m_nValidHeight) << cSrcId << cDesId << VARINT(m_ullFees)
			<< VARINT(m_ullValues) << m_vchContract;

	return cSs.GetHash();
}


bool CRewardTransaction::ExecuteTx(int nIndex, CAccountViewCache &cAcctView, CValidationState &cValidateState, CTxUndo &cTxundo,
		int nHeight, CTransactionDBCache &cTxCache, CScriptDBViewCache &cScriptDB) {

	CID cId(m_cAccount);
	if (m_cAccount.type() != typeid(CRegID)) {
		return cValidateState.DoS(100,
				ERRORMSG(
						"ExecuteTx() : CRewardTransaction ExecuteTx, account %s error, data type must be either CRegID",
						HexStr(cId.GetID())), UPDATE_ACCOUNT_FAIL, "bad-account");
	}
	CAccount cAcctInfo;
	if (!cAcctView.GetAccount(m_cAccount, cAcctInfo)) {
		return cValidateState.DoS(100,
				ERRORMSG("ExecuteTx() : CRewardTransaction ExecuteTx, read source addr %s account info error",
						HexStr(cId.GetID())), UPDATE_ACCOUNT_FAIL, "bad-read-accountdb");
	}
	...
	CUserID cUserId = cAcctInfo.m_cKeyID;
	if (!cAcctView.SetAccount(cUserId, cAcctInfo)) {
		return cValidateState.DoS(100, ERRORMSG("ExecuteTx() : CRewardTransaction ExecuteTx, write secure account info error"),
				UPDATE_ACCOUNT_FAIL, "bad-save-accountdb");
	}
	cTxundo.Clear();
	cTxundo.m_vcAccountLog.push_back(acctInfoLog);
	cTxundo.m_cTxHash = GetHash();
	if (SysCfg().GetAddressToTxFlag() && 0 == nIndex) {
		CScriptDBOperLog cOperAddressToTxLog;
		CKeyID cSendKeyId;
		if (!cAcctView.GetKeyId(m_cAccount, cSendKeyId)) {
...
bool CRewardTransaction::GetAddress(set<CKeyID> &vAddr, CAccountViewCache &cAcctView, CScriptDBViewCache &cScriptDB) {
	CKeyID cKeyId;
	if (m_cAccount.type() == typeid(CRegID)) {
		if (!cAcctView.GetKeyId(m_cAccount, cKeyId)) {
			return false;
		}
		vAddr.insert(cKeyId);
	} else if (m_cAccount.type() == typeid(CPubKey)) {
		CPubKey cPubKey = boost::get<CPubKey>(m_cAccount);
		if (!cPubKey.IsFullyValid()) {
			return false;
		}
		vAddr.insert(cPubKey.GetKeyID());
	}
	return true;
}
...
Object CRewardTransaction::ToJSON(const CAccountViewCache &cAccountView) const {
	Object result;
	CAccountViewCache cAcctView(cAccountView);
	CKeyID cKeyId;
	result.push_back(Pair("hash", GetHash().GetHex()));
	...
	cAcctView.GetKeyId(m_cAccount, cKeyId);
	result.push_back(Pair("addr", cKeyId.ToAddress()));
...
bool CRewardTransaction::CheckTransction(CValidationState &cValidationState, CAccountViewCache &cAcctView,
		CScriptDBViewCache &cScriptDB) {
	return true;
}
...
bool CRegisterAppTx::ExecuteTx(int nIndex, CAccountViewCache &cAcctView, CValidationState &cValidationState, CTxUndo &cTxundo,
		int nHeight, CTransactionDBCache &cTxCache, CScriptDBViewCache &cScriptDB) {
	CID cId(m_cRegAcctId);
	CAccount cAcctInfo;
	CScriptDBOperLog cOperLog;
	if (!cAcctView.GetAccount(m_cRegAcctId, cAcctInfo)) {
		return cValidationState.DoS(100,
				ERRORMSG("ExecuteTx() : CRegisterAppTx ExecuteTx, read regist addr %s account info error",
						HexStr(cId.GetID())), UPDATE_ACCOUNT_FAIL, "bad-read-accountdb");
	}
	...
	if (ullMinusValue > 0) {
		...
		cTxundo.m_vcAccountLog.push_back(cAcctInfoLog);
	}
	cTxundo.m_cTxHash = GetHash();
	...
	try {
		...
		return cValidationState.DoS(100,
				ERRORMSG(
						("ExecuteTx() :CRegisterAppTx ExecuteTx, Unserialize to vmScript error:" + string(e.what())).c_str()),
				UPDATE_ACCOUNT_FAIL, "unserialize-script-error");
	}
	if (!cVMScript.IsValid()) {
		return cValidationState.DoS(100, ERRORMSG("ExecuteTx() : CRegisterAppTx ExecuteTx, vmScript invalid"), UPDATE_ACCOUNT_FAIL,
				"script-check-failed");
	}
	std::tuple<bool, string> result = CVmlua::syntaxcheck(false, (char *) &cVMScript.m_vuchRom[0],
			cVMScript.m_vuchRom.size());
	bool bOK = std::get<0>(result);
	if (!bOK) {
		return cValidationState.DoS(100,
				ERRORMSG("ExecuteTx() : CRegisterAppTx ExecuteTx, vmScript invalid:%s", std::get<1>(result)),
	...
	if (!cScriptDB.SetScript(cRegId, m_vchScript)) {
		return cValidationState.DoS(100,
				ERRORMSG("ExecuteTx() : CRegisterAppTx ExecuteTx, save script id %s script info error",
						cRegId.ToString()), UPDATE_ACCOUNT_FAIL, "bad-save-scriptdb");
	}
	if (!cAcctView.SaveAccountInfo(cRegId, cKeyId, cAccount)) {
		return cValidationState.DoS(100,
	...
	if (!cOperLog.m_vchKey.empty()) {
		cTxundo.m_vcScriptOperLog.push_back(cOperLog);
	}
	CUserID userId = cAcctInfo.m_cKeyID;
	if (!cAcctView.SetAccount(userId, cAcctInfo)) {
		return cValidationState.DoS(100, ERRORMSG("ExecuteTx() : CRegisterAppTx ExecuteTx, save account info error"),
				UPDATE_ACCOUNT_FAIL, "bad-save-accountdb");
	}
	if (SysCfg().GetAddressToTxFlag()) {
		CScriptDBOperLog cOperAddressToTxLog;
		CKeyID cSendKeyId;
		if (!cAcctView.GetKeyId(m_cRegAcctId, cSendKeyId)) {
...
bool CRegisterAppTx::UndoExecuteTx(int nIndex, CAccountViewCache &cAcctView, CValidationState &cValidationState, CTxUndo &cTxundo,
		int nHeight, CTransactionDBCache &cTxCache, CScriptDBViewCache &cScriptDB) {
	CID cId(m_cRegAcctId);
	CAccount cAccount;
	CUserID cUserId;
	if (!cAcctView.GetAccount(m_cRegAcctId, cAccount)) {
		return cValidationState.DoS(100,
				ERRORMSG("UndoUpdateAccount() : CRegisterAppTx UndoExecuteTx, read regist addr %s account info error",
						HexStr(cId.GetID())), UPDATE_ACCOUNT_FAIL, "bad-read-accountdb");
	}
	...
	if (m_vchScript.size() != 6) {
		...
		if (!cScriptDB.EraseScript(cScriptId)) {
			return cValidationState.DoS(100,
					ERRORMSG("UndoUpdateAccount() : CRegisterAppTx UndoExecuteTx, erase script id %s error",
							cScriptId.ToString()), UPDATE_ACCOUNT_FAIL, "erase-script-failed");
		}
		// delete account
		if (!cAcctView.EraseId(cScriptId)) {
			return cValidationState.DoS(100,
					ERRORMSG("UndoUpdateAccount() : CRegisterAppTx UndoExecuteTx, erase script account %s error",
							cScriptId.ToString()), UPDATE_ACCOUNT_FAIL, "erase-appkeyid-failed");
		}
		CKeyID cKeyId = Hash160(cScriptId.GetVec6());
		cUserId = cKeyId;
		if (!cAcctView.EraseAccount(cUserId)) {
			return cValidationState.DoS(100,
					ERRORMSG("UndoUpdateAccount() : CRegisterAppTx UndoExecuteTx, erase script account %s error",
							cScriptId.ToString()), UPDATE_ACCOUNT_FAIL, "erase-appaccount-failed");
		}
		// LogPrint("INFO", "Delete regid %s app account\n", scriptId.ToString());
	}

	for (auto &itemLog : cTxundo.m_vcAccountLog) {
		if (itemLog.m_cKeyID == cAccount.m_cKeyID) {
			if (!cAccount.UndoOperateAccount(itemLog)) {
				return cValidationState.DoS(100,
						ERRORMSG(
								"UndoUpdateAccount: CRegisterAppTx UndoExecuteTx, undo operate cAccount error, cKeyId=%s",
								cAccount.m_cKeyID.ToString()), UPDATE_ACCOUNT_FAIL, "undo-cAccount-failed");
			}
		}
	}

	vector<CScriptDBOperLog>::reverse_iterator rIterScriptDBLog = cTxundo.m_vcScriptOperLog.rbegin();
	for (; rIterScriptDBLog != cTxundo.m_vcScriptOperLog.rend(); ++rIterScriptDBLog) {
		if (!cScriptDB.UndoScriptData(rIterScriptDBLog->m_vchKey, rIterScriptDBLog->m_vchValue))
			return cValidationState.DoS(100, ERRORMSG("ExecuteTx() : CRegisterAppTx UndoExecuteTx, undo scriptdb data error"),
					UPDATE_ACCOUNT_FAIL, "undo-scriptdb-failed");
	}
	cUserId = cAccount.m_cKeyID;
	if (!cAcctView.SetAccount(cUserId, cAccount)) {
		return cValidationState.DoS(100, ERRORMSG("ExecuteTx() : CRegisterAppTx UndoExecuteTx, save account error"),
				UPDATE_ACCOUNT_FAIL, "bad-save-accountdb");
	}
	return true;
}
...
Object CRegisterAppTx::ToJSON(const CAccountViewCache &cAccountView) const{
	Object result;
	CAccountViewCache cAcctView(cAccountView);
    CKeyID cKeyId;
	result.push_back(Pair("hash", GetHash().GetHex()));
	...
	cAcctView.GetKeyId(m_cRegAcctId, cKeyId);
	result.push_back(Pair("addr", cKeyId.ToAddress()));
...
bool CRegisterAppTx::CheckTransction(CValidationState &cValidationState, CAccountViewCache &cAcctView,
		CScriptDBViewCache &cScriptDB) {
	if (m_cRegAcctId.type() != typeid(CRegID)) {
		return cValidationState.DoS(100, ERRORMSG("CheckTransaction() : CRegisterAppTx regAcctId must be CRegID"),
				REJECT_INVALID, "regacctid-type-error");
	}

	if (!MoneyRange(m_ullFees)) {
		return cValidationState.DoS(100,
				ERRORMSG("CheckTransaction() : CRegisterAppTx CheckTransction, tx fee out of range"), REJECT_INVALID,
				"fee-too-large");
	}

	uint64_t llFuel = ceil(m_vchScript.size() / 100) * GetFuelRate(cScriptDB);
	if (llFuel < 1 * COIN) {
		llFuel = 1 * COIN;
	}
	...
	if (m_ullFees < llFuel) {
		return cValidationState.DoS(100,
				ERRORMSG(
						"CheckTransaction() : CRegisterAppTx CheckTransction, register app tx fee too litter (actual:%lld vs need:%lld)",
						m_ullFees, llFuel), REJECT_INVALID, "fee-too-litter");
	}

	CAccount cAcctInfo;
	if (!cAcctView.GetAccount(boost::get<CRegID>(m_cRegAcctId), cAcctInfo)) {
		return cValidationState.DoS(100,
				ERRORMSG("CheckTransaction() : CRegisterAppTx CheckTransction, get account falied"), REJECT_INVALID,
				"bad-getaccount");
	}
	if (!cAcctInfo.IsRegister()) {
		return cValidationState.DoS(100,
				ERRORMSG("CheckTransaction(): CRegisterAppTx CheckTransction, account have not registed public key"),
				REJECT_INVALID, "bad-no-pubkey");
	}
	uint256 cSignhash = SignatureHash();
	if (!CheckSignScript(cSignhash, m_vchSignature, cAcctInfo.m_cPublicKey)) {
		return cValidationState.DoS(100,
				ERRORMSG("CheckTransaction() : CRegisterAppTx CheckTransction, CheckSignScript failed"), REJECT_INVALID,
				"bad-signscript-check");
	}
	return true;
}
...
uint256 CRegisterAppTx::SignatureHash() const {
	CHashWriter cSs(SER_GETHASH, 0);
	CID cRegAccId(m_cRegAcctId);
	cSs << VARINT(m_nVersion) << m_chTxType << VARINT(m_nValidHeight) << cRegAccId << m_vchScript << VARINT(m_ullFees);
...
bool CTxUndo::GetAccountOperLog(const CKeyID &cKeyId, CAccountLog &cAccountLog) {
...
bool CAccount::UndoOperateAccount(const CAccountLog & cAccountLog) {
	LogPrint("undo_account", "after operate:%s\n", ToString());
	m_ullValues = cAccountLog.m_ullValues;
	m_nHeight = cAccountLog.m_nHeight;
	m_ullCoinDay = cAccountLog.m_ullCoinDay;
...
bool CAccount::IsMoneyOverflow(uint64_t ullAddMoney) {
	if (!MoneyRange(ullAddMoney)) {
		return ERRORMSG("money:%lld too larger than MaxMoney");
	}
	return true;
}

bool CAccount::OperateAccount(emOperType type, const uint64_t &ullValue, const int nCurHeight) {
	LogPrint("op_account", "before operate:%s\n", ToString());

	if (!IsMoneyOverflow(ullValue)) {
		return false;
	}
	...
	case EM_MINUS_FREE: {
		...
		arith_uint256 value256 = (arith_uint256) ullValue;
		...
		uint64_t ullRemainCoinDay = remainCoinDay256.GetLow64();
