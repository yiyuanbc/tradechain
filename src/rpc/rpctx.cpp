...
Value listtransactions(const Array& params, bool fHelp) {
	...
	for (auto const &wtx : g_pwalletMain->m_mapInBlockTx) {
		CBlockIndex *pIndex = g_mapBlockIndex[wtx.first];
		if (pIndex != NULL)
			blockInfoMap.insert(make_pair(pIndex->m_nHeight, wtx.first));
	}

	int txnCount(0);
	int nIndex(0);
	for (auto const &wtx : blockInfoMap) {
		CAccountTx accountTx = g_pwalletMain->m_mapInBlockTx[wtx.second];
		for (auto const & item : accountTx.m_mapAccountTx) {

			if(item.second->m_chTxType == EM_COMMON_TX ||
					item.second->m_chTxType == EM_CONTRACT_TX) {

				CTransaction* ptx = (CTransaction*)item.second.get();
				CKeyID SendKeyID;

				CRegID sendRegID = boost:: get < CRegID > (ptx->m_cSrcRegId);
				SendKeyID = sendRegID.getKeyID(*g_pAccountViewTip);

				CKeyID RecvKeyID;
				if (ptx->m_cDesUserId.type() == typeid(CKeyID)) {
					RecvKeyID = boost::get<CKeyID>(ptx->m_cDesUserId);
				} else if (ptx->m_cDesUserId.type() == typeid(CRegID)) {
					CRegID desRegID = boost::get<CRegID>(ptx->m_cDesUserId);
					RecvKeyID = desRegID.getKeyID(*g_pAccountViewTip);
				}
				...
				if(bSend) {
					if(g_pwalletMain->HaveKey(SendKeyID)) {
						Object obj;
						...
						obj.push_back(Pair("confirmations", g_cChainActive.Tip()->m_nHeight - accountTx.m_nBlockhigh));
						obj.push_back(Pair("blockhash", (g_cChainActive[accountTx.m_nBlockhigh]->GetBlockHash().GetHex())));
						obj.push_back(Pair("blocktime", (int64_t)(g_cChainActive[accountTx.m_nBlockhigh]->m_unTime)));
						obj.push_back(Pair("txid", item.second->GetHash().GetHex()));

						if(item.second->m_chTxType == EM_CONTRACT_TX) {
							obj.push_back(Pair("txtype", "CONTRACT_TX"));
							obj.push_back(Pair("contract", HexStr(ptx->m_vchContract)));
						} else {
							obj.push_back(Pair("txtype", "COMMON_TX"));
				...
				if(bRecv) {
					if(g_pwalletMain->HaveKey(RecvKeyID)) {
						Object obj;
	...
	for (auto const &wtx : g_pwalletMain->m_mapInBlockTx) {
		for (auto const & item : wtx.second.m_mapAccountTx) {
			...
			if(item.second->m_chTxType == EM_COMMON_TX) {
				...
				if( !accView.GetKeyId(ptx->m_cSrcRegId, keyid)) {
					continue;
				}
				string srcAddr = keyid.ToAddress();
				if( !accView.GetKeyId(ptx->m_cDesUserId, keyid)) {
					continue;
				}
				...
				double dAmount = static_cast<double>(item.second->GetValue()) / COIN;
				obj.push_back(Pair("amount", dAmount));
				obj.push_back(Pair("confirmations", g_cChainActive.Tip()->m_nHeight - wtx.second.m_nBlockhigh));
				obj.push_back(Pair("blockhash", (g_cChainActive[wtx.second.m_nBlockhigh]->GetBlockHash().GetHex())));
				obj.push_back(Pair("blocktime", (int64_t)(g_cChainActive[wtx.second.m_nBlockhigh]->m_unTime)));
				obj.push_back(Pair("txid", item.second->GetHash().GetHex()));
				arrayData.push_back(obj);
			}
		}
	...
	for (auto const &wtx : blockInfoMap) {
		...
		for (auto const & item : accountTx.m_mapAccountTx) {
			...
			if (item.second->m_chTxType == EM_CONTRACT_TX) {
				...
				if(strRegId != getregidstring(ptx->m_cDesUserId))
					continue;
				...
				obj.push_back(Pair("regid",  getregidstring(ptx->m_cSrcRegId)));
				accView.GetKeyId(ptx->m_cSrcRegId, keyid);
				obj.push_back(Pair("addr",  keyid.ToAddress()));
				obj.push_back(Pair("desregid", getregidstring(ptx->m_cDesUserId)));
				accView.GetKeyId(ptx->m_cDesUserId, keyid);
				obj.push_back(Pair("desaddr", keyid.ToAddress()));
				obj.push_back(Pair("money", ptx->m_ullValues));
				obj.push_back(Pair("fees", ptx->m_ullFees));
				obj.push_back(Pair("height", ptx->m_nValidHeight));
				obj.push_back(Pair("Contract", HexStr(ptx->m_vchContract)));
				arrayData.push_back(obj);
...
if (fHelp || params.size() > 2) {
	...
	for (auto const &blockInfo : blockInfoMap) {
		CAccountTx accountTx = g_pwalletMain->m_mapInBlockTx[blockInfo.second];
		for (auto const & item : accountTx.m_mapAccountTx) {
			if (nFrom-- > 0)
				continue;
			if (++nCount > nDefCount) {
