...
Value setgenerate(const Array& params, bool bHelp) {
	...
	Object obj;
	if (bGenerate == false) {
		GenerateCoinBlock(false, g_pwalletMain, 1);
		obj.push_back(Pair("msg", "stoping  mining"));
		return obj;
	}

	GenerateCoinBlock(true, g_pwalletMain, nGenProcLimit); //跑完之后需要退出
...
Value submitblock(const Array& params, bool bHelp) {
	if (bHelp || params.size() < 1 || params.size() > 2) {
		throw runtime_error(
				"submitblock \"hexdata\" ( \"jsonparametersobject\" )\n"
						...
						"See https://en.coin.it/wiki/BIP_0022 for full specification.\n"
	...
	bool bAccepted = ProcessBlock(cValidationState, NULL, &cBlock);
	Object obj;
	if (!bAccepted) {
		...
		obj.push_back(Pair("hash", cBlock.GetHash().ToString()));
	}
	return obj;
}
