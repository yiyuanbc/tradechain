...
template<typename T>
void ConvertTo(Value& value, bool bAllowNull = false) {
	if (bAllowNull && value.type() == null_type) {
		return;
	}
...
Array RPCConvertValues(const string &strMethod, const vector<string> &strParams) {
    ...
    if (strMethod == "getcoinstate"          && n > 0) ConvertTo<int>(params[0]);
 ...
 //   if (strMethod == "registaccounttx"          && n > 2) ConvertTo<bool>(params[2]);
	...
	if (strMethod == "listcontracttx"           && n > 1) ConvertTo<int>(params[1]);
    if (strMethod == "listcontracttx"           && n > 2) ConvertTo<int>(params[2]);
    return params;
}
...
int CommandLineRPC(int argc, char *argv[]) {
	...
	try {
		...
		vector<string> vstrParams(&argv[2], &argv[argc]);
