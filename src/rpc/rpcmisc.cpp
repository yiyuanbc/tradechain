...
Value getbalancev2(const Array& params, bool fHelp)
{
	int size = params.size();
		...
		Object obj;
		if (size == 0) {
			return ValueFromAmount(g_pwalletMain->GetRawBalance());
		} else if (size == 1) {
			...
			if (addr == "*") {
				if (0 != nConf) {
					...
					while (nConf) {
						if (g_pwalletMain->m_mapInBlockTx.count(pBlockIndex->GetBlockHash()) > 0) {
							map<uint256, std::shared_ptr<CBaseTransaction> > mapTx = g_pwalletMain->m_mapInBlockTx[pBlockIndex->GetBlockHash()].m_mapAccountTx;
							for (auto &item : mapTx) {
								if (EM_COMMON_TX == item.second->m_chTxType) {
									CTransaction *pTx = (CTransaction *) item.second.get();
									CKeyID srcKeyId, desKeyId;
									g_pAccountViewTip->GetKeyId(pTx->m_cSrcRegId, srcKeyId);
									g_pAccountViewTip->GetKeyId(pTx->m_cDesUserId, desKeyId);
									if (!g_pwalletMain->HaveKey(srcKeyId) && g_pwalletMain->HaveKey(desKeyId)) {
										nValue = pTx->m_ullValues;
									}
								}
							}
						}
					...
					return ValueFromAmount(g_pwalletMain->GetRawBalance() - nValue);
				} else {
					return ValueFromAmount(g_pwalletMain->GetRawBalance(false));
				}
			} else {
				CKeyID keyid;
				if (!GetKeyId(addr, keyid)) {
				...
				if (g_pwalletMain->HaveKey(keyid)) {
					if (0 != nConf) {
						...
						return ValueFromAmount(g_pAccountViewTip->GetRawBalance(keyid) - nValue);
					} else {
						return ValueFromAmount(g_cTxMemPool.m_pAccountViewCache->GetRawBalance(keyid));
					}
				} else {
					throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "address not inwallet");
				}
...
Value verifymessage(const Array& params, bool bHelp) {
    if (bHelp || params.size() != 3) {
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"Coinaddress or pubkey\"  (string, required) The Coin address to use for the signature.\n"
