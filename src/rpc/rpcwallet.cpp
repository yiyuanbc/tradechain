...
Value signmessage(const Array& params, bool bHelp) {
    if (bHelp || params.size() != 2) {
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"Coinaddress\"  (string, required) The Coin address to use for the private key.\n"
...
typedef struct {
	...
	unsigned char arruchAddress[34]; //  ת�˵�ַ
...
Value sendtoaddressv2(const Array& params, bool fHelp)
 {
	...
	for (auto &te : sKeyid) {
		if (g_pAccountViewTip->GetRawBalance(te) >= nAmount + SysCfg().GetTxFee()) {
			if (g_pAccountViewTip->GetRegId(CUserID(te), sendreg)) {
				sendKeyId = te;
				break;
			}
		}
	...
	if (!g_pAccountViewTip->GetRegId(CUserID(sendKeyId), sendreg)) {
		throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid  address");
	}

	if (g_pAccountViewTip->GetRegId(CUserID(RevKeyId), revreg)) {
		rev = revreg;
	} else {
		rev = RevKeyId;
	}

	CTransaction tx(sendreg, rev, SysCfg().GetTxFee(), nAmount, g_cChainActive.Height());

	if (!g_pwalletMain->Sign(sendKeyId, tx.SignatureHash(), tx.m_vchSignature)) {
		throw JSONRPCError(RPC_INVALID_PARAMETER,  "Sign failed");
	}
...
Value sendtoaddress(const Array& params, bool bHelp) {
	...
	if (bHelp || (!(nSize == 2 || nSize == 3))) {
		throw runtime_error(
						"sendtoaddress (\"Coinaddress\") \"receive address\" \"amount\"\n"
						"\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n"
						+ HelpRequiringPassphrase() + "\nArguments:\n"
						"1. \"Coinaddress\"  (string, optional) The Coin address to send to.\n"
...
{
	  if (g_pwalletMain->IsCrypted() && (fHelp || params.size() != 2))
	        throw runtime_error(
	    ...
	    RPCRunLater("lockwallet", boost::bind(LockWallet, g_pwalletMain), nSleepTime);
	    return null_type;
}
...
Value walletpassphrase(const Array& params, bool bHelp) {
    if (g_pwalletMain->IsCrypted() && (bHelp || params.size() != 2)) {
        throw runtime_error(
            "walletpassphrase \"passphrase\" timeout\n"
            "\nStores the wallet decryption key in memory for 'timeout' seconds.\n"
            "This is needed prior to performing transactions related to private keys such as sending Coin\n"
            "\nArguments:\n"
            "1. \"passphrase\"     (string, required) The wallet passphrase\n"
            "2. timeout            (numeric, required) The time to keep the decryption key in seconds.\n"
            "\nNote:\n"
...
{
    if (g_pwalletMain->IsCrypted() && (fHelp || params.size() != 0))
        throw runtime_error(
            "walletlock\n"
...
Value encryptwallet(const Array& params, bool bHelp) {
    if (!g_pwalletMain->IsCrypted() && (bHelp || params.size() != 1)) {
        throw runtime_error(
            ...
            + HelpExampleCli("signmessage", "\"Coinaddress\" \"test message\"") +
