...
Value importprivkey(const Array& params, bool bHelp) {
	if (bHelp || params.size() < 1 || params.size() > 3) {
		throw runtime_error(
				"importprivkey \"Coin privkey\" ( \"label\" rescan )\n"
						"\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
						"\nArguments:\n"
						"1. \"Coin privkey\"   (string, required) The private key (see dumpprivkey)\n"
	...
	if (strSecret.length() == 32) {
		...
		CCoinSecret cCoinSecret;
		bool bGood = cCoinSecret.SetString(strSecret);
...
Value dumpprivkey(const Array& params, bool bHelp) {
	...
	CCoinAddress cCoinAddress;
	if (!cCoinAddress.SetString(strAddress)) {
		throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Coin address");
	}
	CKeyID cKeyID;
	if (!cCoinAddress.GetKeyID(cKeyID)) {
	...
	Object reply;
	reply.push_back(Pair("privkey", CCoinSecret(cSecret).ToString()));
	if (cMinerKey.ToString() != cSecret.ToString()) {
		reply.push_back(Pair("minerkey", CCoinSecret(cMinerKey).ToString()));
	} else {
...
Value dumpwallet(const Array& params, bool bHelp) {
	...
	Object reply;
	reply.push_back(Pair("created by Coin", g_strClientBuild + g_strClientDate));
