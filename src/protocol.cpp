...
CMessageHeader::CMessageHeader(const char* pszCommand, unsigned int unMessageSizeIn) {
	memcpy(m_pchMessageStart, SysCfg().MessageStart(), MESSAGE_START_SIZE);
	strncpy(m_pchCommand, pszCommand, COMMAND_SIZE);
	m_unMessageSize = unMessageSizeIn;
	m_unChecksum = 0;
}
...
CAddress::CAddress(CService cIpIn, uint64_t ullServicesIn) : CService(cIpIn) {
	Init();
	m_ullServices = ullServicesIn;
}
