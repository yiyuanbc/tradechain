...
class CRewardTransaction: public CBaseTransaction {
 public:
	...
	bool GetAddress(set<CKeyID> &vAddr, CAccountViewCache &cAcctView, CScriptDBViewCache &cScriptDB);

	bool ExecuteTx(int nIndex, CAccountViewCache &cAcctView, CValidationState &cValidateState, CTxUndo &cTxundo, int nHeight,
			CTransactionDBCache &cTxCache, CScriptDBViewCache &cScriptDB);

	bool CheckTransction(CValidationState &cValidationState, CAccountViewCache &cAcctView, CScriptDBViewCache &cScriptDB);
...
class CRegisterAppTx: public CBaseTransaction {
 public:
	...
	bool ExecuteTx(int nIndex, CAccountViewCache &cAcctView, CValidationState &cValidationState, CTxUndo &cTxundo,
			int nHeight, CTransactionDBCache &cTxCache, CScriptDBViewCache &cScriptDB);

	bool UndoExecuteTx(int nIndex, CAccountViewCache &cAcctView, CValidationState &cValidationState, CTxUndo &cTxundo,
			int nHeight, CTransactionDBCache &cTxCache, CScriptDBViewCache &cScriptDB);

	bool CheckTransction(CValidationState &cValidationState, CAccountViewCache &cAcctView,
...
class CTxUndo {
 public:
	bool GetAccountOperLog(const CKeyID &keyId, CAccountLog &cAccountLog);
	void Clear() {
		m_cTxHash = uint256();
...
class CAccount {
 public :
	 ...
	 */
	bool OperateAccount(emOperType type, const uint64_t &ullValue,const int nCurHeight);

	bool UndoOperateAccount(const CAccountLog & cAccountLog);
 ...
 private:
	bool IsMoneyOverflow(uint64_t ullAddMoney);
};
