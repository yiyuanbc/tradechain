...
#ifndef COIN_BIGNUM_H_
#define COIN_BIGNUM_H_
...
class bignum_error: public runtime_error {
 public:
	explicit bignum_error(const string& str) :
			runtime_error(str) {
	}
};
...
class CBigNum: public BIGNUM {
 public:
	...
	void setuint64(uint64_t n) {
		...
		unsigned int unSize = p - (pch + 4);
		pch[0] = (unSize >> 24) & 0xff;
		pch[1] = (unSize >> 16) & 0xff;
		pch[2] = (unSize >> 8) & 0xff;
		pch[3] = (unSize) & 0xff;
		BN_mpi2bn(pch, p - pch, this);
	}
	...
	void setvch(const vector<unsigned char>& vch) {
		...
		unsigned int unSize = vch.size();
		// BIGNUM's byte stream format expects 4 bytes of
		// big endian size data info at the front
		vch2[0] = (unSize >> 24) & 0xff;
		vch2[1] = (unSize >> 16) & 0xff;
		vch2[2] = (unSize >> 8) & 0xff;
		vch2[3] = (unSize >> 0) & 0xff;
	...
	vector<unsigned char> getvch() const {
		unsigned int unSize = BN_bn2mpi(this, NULL);
		if (unSize <= 4) {
			return vector<unsigned char>();
		}

		vector<unsigned char> vch(unSize);
		BN_bn2mpi(this, &vch[0]);
	...
	//
	// coin only uses this "compact" format for encoding difficulty
	...
	CBigNum& SetCompact(unsigned int nCompact) {
		unsigned int unSize = nCompact >> 24;
		bool bNegative = (nCompact & 0x00800000) != 0;
		unsigned int unWord = nCompact & 0x007fffff;

		if (unSize <= 3) {
			unWord >>= 8 * (3 - unSize);
			BN_set_word(this, unWord);
		} else {
			BN_set_word(this, unWord);
			BN_lshift(this, this, 8 * (unSize - 3));
		}
		BN_set_negative(this, bNegative);

		return *this;
	}

	unsigned int GetCompact() const {
		unsigned int unSize = BN_num_bytes(this);
		unsigned int unCompact = 0;
		if (unSize <= 3) {
			unCompact = BN_get_word(this) << 8 * (3 - unSize);
		} else {
			CBigNum bn;
			BN_rshift(&bn, this, 8 * (unSize - 3));
			unCompact = BN_get_word(&bn);
		}

		// The 0x00800000 bit denotes the sign.
		// Thus, if it is already set, divide the mantissa by 256 and increase the exponent.
		if (unCompact & 0x00800000) {
			unCompact >>= 8;
			unSize++;
		}
		unCompact |= unSize << 24;
		unCompact |= (BN_is_negative(this) ? 0x00800000 : 0);
