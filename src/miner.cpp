...
uint256 GetAdjustHash(const uint256 cTargetHash, const uint64_t ullPos, const int nCurHeight) {
	uint64_t ullPosAcc = ullPos / COIN;
	ullPosAcc /= SysCfg().GetIntervalPos();
	ullPosAcc = max(ullPosAcc, (uint64_t) 1);
	arith_uint256 cAdjustHash = UintToArith256(cTargetHash); //adjust nbits
...
bool CreatePosTx(const CBlockIndex *pPrevIndex, CBlock *pBlock, set<CKeyID>&setCreateKey,
		CAccountViewCache &cAccountViewCache, CTransactionDBCache &cTxCache, CScriptDBViewCache &cScriptCache) {
	set<CKeyID> setKeyID;
	setKeyID.clear();
	...
	set<CAccount, ST_AccountComparator> setAcctInfo;
	{
		...
		for (const auto &keyid : setKeyID) {                             //second:get account by keyID
			...
			if (cAccountViewCache.GetAccount(userId, cAccountInfo)) {   // check acctInfo is or not allowed to mining ,
				...
				if (cAccountInfo.IsRegister() && cAccountInfo.GetAccountPos(pBlock->GetHeight()) > 0
						&& cAccountInfo.IsMiner(pBlock->GetHeight())) {
...
bool VerifyPosTx(CAccountViewCache &cAccountViewCache, const CBlock *pBlock, CTransactionDBCache &cTxDBCache,
	...
	if (bNeedRunTx) {
		...
		for (unsigned int i = 1; i < pBlock->vptx.size(); i++) {
			...
			if (!pBaseTx->ExecuteTx(i, cAccountView, cValidState, cTxUndo, pBlock->GetHeight(), cTxDBCache,
					cScriptDBView)) {
...
ST_BlockTemplate* CreateNewBlock(CAccountViewCache &cAccViewCache, CTransactionDBCache &cTxCache,
	...
	int64_t llFees = 0;
	{
		...
		((CRewardTransaction*) pBlock->vptx[0].get())->m_ullRewardValue = llFees - llTotalFuel + GetBlockSubsidy(pIndexPrev->m_nHeight + 1);
...
bool static MiningBlock(CBlock *pBlock, CWallet *pWallet, CBlockIndex* pBlockIndexPrev,
	...
	while (true) {
		...
		if (g_vNodes.empty() && SysCfg().NetworkID() != EM_REGTEST) {
			return false;
		}
		...
		bool bIncreatedfalg = CreatePosTx(pBlockIndexPrev, pBlock, setCreateKey, cAccountViewCache, cTxDBCache,
				scriptCache);
...
void static CoinMiner(CWallet *pWallet,int nTargetConter) {
	...
	try {
	       SetMinerStatus(true);
		while (true) {
			if (SysCfg().NetworkID() != EM_REGTEST) {
...
void GenerateCoinBlock(bool bGenerate, CWallet* pWallet, int nThreads) {
	...
	minerThreads->create_thread(boost::bind(&CoinMiner, pWallet, nThreads));
}
