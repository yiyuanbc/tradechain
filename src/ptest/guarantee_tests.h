...
typedef struct {
	...
	uint64_t ullArbiterMoneyX;             	//!<仲裁费用X
	uint64_t ullOvertimeMoneyYmax;  		//!<超时未判决的最大赔偿费用Y
	uint64_t ullConfigMoneyZ;              	//!<无争议裁决费用Z
	unsigned int  unOvertimeheightT;  		//!<判决期限时间T
...
}ST_TX_REGISTER_CONTRACT;  //!<注册仲裁账户
...
typedef struct {
	...
	unsigned int unHeight;           		//!<每个交易环节的超时高度
...
typedef struct {
	...
	uint64_t ullWinnerMoney;            	//!<最终获得的金额
	char  arrchLoser[6];       				//!<输家ID（采用6字节的账户ID）
	uint64_t ullLoserMoney;            		//!<最终获得的金额
	IMPLEMENT_SERIALIZE
	(
		READWRITE(uchType);
			for (int i = 0; i < 32; i++) {
