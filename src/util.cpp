...
int LogPrintStr(const char* pszCategory, const string &strLog) {
	...
	if (SysCfg().IsPrintToFile()) {
		...
		cPathDebug = GetDataDir() / strFile;   // /home/share/bille/coin_test/regtest/INFO.log
...
static boost::filesystem::path g_sPathCached[EM_MAX_NETWORK_TYPES + 1];
...
void ClearDatadirCache() {
	fill(&g_sPathCached[0], &g_sPathCached[EM_MAX_NETWORK_TYPES + 1], boost::filesystem::path());
}

boost::filesystem::path GetConfigFile() {
	boost::filesystem::path cPathConfigFile(CBaseParams::GetArg("-conf", IniCfg().GetCoinName()+".conf"));
...
void ReadConfigFile(map<string, string>& mapSettingsRet, map<string, vector<string> >& mapMultiSettingsRet) {
	...
	if (!streamConfig.good()) {
		return; // No Coin.conf file is OK
	}
	...
	for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
		// Don't overwrite existing settings so command line settings override Coin.conf
...
//
static int64_t g_llMockTime = 0;  // For unit testing
...
// Format the subversion field according to BIP 14 spec (https://en.Coin.it/wiki/BIP_0014)
